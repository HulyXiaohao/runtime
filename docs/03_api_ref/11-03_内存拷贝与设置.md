...
初始化内存，将内存中的内容设置为指定的32位无符号整数值。
与 aclrtMemset 的区别在于：本接口以 32位无符号整数 为单位进行填充，N 表示要设置的 uint32_t 元素个数。

### 参数说明
| 参数名 | 输入/输出 | 说明 |
| --- | :---: | --- |
| ptr | 输入 | 内存起始地址的指针。 |
| memSize | 输入 | 内存的最大长度，单位 Byte（必须 ≥ N * 4）。 |
| value | 输入 | 要填充的32位无符号整数值。 |
| N | 输入 | 需要设置的 uint32_t 元素个数。 |

### 返回值说明
返回 ACL_SUCCESS 表示成功，返回其他值表示失败，请参见[aclError](25_数据类型及其操作接口.md#aclError)。

### 约束说明

本接口会立即执行内存初始化。
函数内部不会进行隐式的 device 同步或流同步。
若申请的内存不是通过 ACL 接口（aclrtMallocHost/aclrtMalloc）申请的，可能导致未定义行为。
...
初始化内存，将内存中的内容设置为指定的32位无符号整数值。异步接口。
与 aclrtMemsetAsync 的区别：本接口以 uint32_t 元素为单位进行填充，N 表示元素个数。
...
| value | 输入 | 要填充的32位无符号整数值。 |
| N | 输入 | 需要设置的 uint32_t 元素个数。 |
| stream | 输入 | 指定执行内存初始化任务的 Stream。类型定义请参见[aclrtStream](25_数据类型及其操作接口.md#aclrtStream)。 |
...
对 Device 内存的填充任务在指定 Stream 上异步执行，调用方需确保 Stream 有效且未被销毁。
对 Host 内存，接口行为与同步版本一致（立即填充，不依赖 Stream）。
若 stream 为 nullptr，将使用当前 Context 的默认流。
若申请的内存不是通过 ACL 接口（aclrtMallocHost/aclrtMalloc）申请的，可能导致未定义行为。